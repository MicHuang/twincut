...
| # | 任务 | owner | status | 备注 |
|---|------|-------|--------|------|
| F-H5 | stage9 apply exit-code assertions + keep-policy test polish | claude@mac-joyce | in-progress | Test-only hygiene bundling the recorded "Next up" polish: (1) replace the 14 `|| true` invocation masks in `tests/p1_stage9_smoke.sh` with captured-rc assertions (characterize actual CLI exit codes first; touch all sections together per gemini MINOR); (2) K3 in `tests/keep_policy_smoke.sh` also asserts `dup_group.keep_path` JSON like K2; (3) note that the equal-mtime pin discriminates on ext4 (APFS find order coincides with byte order). DoD: `bash tests/p1_stage9_smoke.sh` + `bash tests/keep_policy_smoke.sh` + `make test` green, shellcheck CI gate clean, PR + user merge. Blocked-if: a masked invocation turns out to exit nonzero by design ambiguity needing a product-behavior decision. |
