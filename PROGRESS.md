...
**Current milestone:** 🔍 IN REVIEW — **I-1 Web UI i18n (en + zh-Hans)** is implemented and internally reviewed on `claude/i18n-zh-hans`; awaiting Tier-1 third-party review, then user merge approval. This is the 2026-05-15 design spec's §10 stage 8, which never landed because the implementation renumbered the stages (the shipped "Stage 8" was the Thumbnail-detect UI), leaving a stated user requirement on no board for ~4 months. 23 commits, 42 files, ~5000 insertions. The bash/NDJSON event contract was not touched: `git diff main...HEAD -- bin/ lib/ tests/fixtures/` is empty, verified independently at three separate points.

**Found incidentally, and more urgent than this feature — recommended as the next task:** `bin/twincut.sh`'s `SELF_DIR` resolution does not resolve a symlink at the final path component, so invoking the installed `~/.local/bin/twincut` symlink leaves `LIB_DIR` empty and `lib/events.sh` is never sourced. Reproduced: the repo script emits `run_start`/`run_end`, the installed symlink emits zero events and prints `emit_run_start: command not found`. Since `installers/install.sh` creates exactly that symlink and the Web UI's default `--twincut-bin` resolution finds it on PATH, **an installed user's Web UI has no working event channel at all** — silently, because the plain-text report still works. No CI job exercises the installed-symlink path.
...
| # | 任务 | owner | status | 备注 |
|---|------|-------|--------|------|
| I-1 | Web UI i18n (en + zh-Hans) | claude@macmini-yiqi | in-review | Bilingual switchable UI on `claude/i18n-zh-hans`. One pre-parsed template set per locale binds `t`/`tmap`/`lang` at parse time, so the 24 `ExecuteTemplate` sites became `s.tmplFor(r)` with no view-struct change and a mixed-language render is structurally impossible. Resolution `lang` cookie → `--lang` → `Accept-Language` → `en` (revives a flag dead since it was added). 175 catalog keys per locale, key-set parity and non-emptiness enforced at startup; a scanner test plus a tmap-subkey contract test make a forgotten translation fail CI. `httpErrorT` covers user-triggerable 4xx only; 5xx keep raw Go errors by design. Executed via superpowers:subagent-driven-development: 11 tasks, 11 task reviews, 6 fix rounds, a whole-branch review and one fix wave. Implementation surfaced 8 defects in my own plan/spec/recipe — the worst would have shipped a broken results page (a top-level `const` colliding on htmx re-execution) and a behavior regression (a plan `-` line quoting a baseline that never existed, which collapsed `video_fast`/`video_strict` into one label). 15 controller rulings, 2 reversing my own earlier calls; all confirmed by the whole-branch review. Pending: Tier-1, user merge approval. |
...
| initial-agent-team-adoption | docs/progress/archive/initial-agent-team-adoption.md | archived | rotated 2026-07-30, 2 entries |

---

## Handoff Log  _(append only, newest on top)_

### 2026-09-10 `[claude]@macmini-yiqi` — I-1 implemented and internally reviewed; awaiting Tier-1 + merge
- **Scope.** Web UI i18n (en + zh-Hans) with a header switcher, per spec `docs/superpowers/specs/2026-09-09-twincut-i18n-design.md` and plan `docs/superpowers/plans/2026-09-09-twincut-i18n.md`. Revives the 2026-05-15 spec's stage 8, orphaned for ~4 months by a stage renumbering.
- **Architecture.** `t`/`tmap`/`lang` bound into a per-locale FuncMap at parse time rather than threaded through data, so no view struct or template-data shape changed and a handler cannot render a mixed-language page. Translation lives entirely in the Go display layer: the bash/NDJSON contract and its fixtures are untouched, verified three times independently.
- **Verification.** `make test` + `make build` green; `go test ./... -count=1`, `go vet`, `gofmt -l` clean; `bash -n` and `git diff --check` silent. The final gate mutation-tested the safety net in a scratch copy: deleting a required bridge subkey, or corrupting an English catalog value, now fails red — both passed green before the fix wave. shellcheck is CI-only on this machine and was not run locally.
- **Process residue worth keeping.** Implementation found 8 defects in my own plan/spec/recipe, two of them shippable bugs. 4 quota interruptions cost zero work because the tree was verified before each re-dispatch. An implementer destroyed `~/.twincut-ui` (recents + run journals, unrecoverable) as "scratch cleanup"; restore capability survived because manifests live in the quarantine dirs. `CLAUDE.md` now carries a structural guardrail (`--state-dir`, commit `f9909ea`) rather than a prohibition.
- **Next for whoever picks this up.** (1) Tier-1 third-party review, then user merge approval. (2) **Then fix the installed-symlink `LIB_DIR` bug described in the Status Board — it is more urgent than anything in this feature.** (3) The user still owes a bilingual copy sign-off; open items are ellipsis convention, 文件夹/目录 for the same quarantine location, and an ASCII space before the Chinese measure word at 5 uncited sites.
...
- PR body records root causes, behavior impact, red/mutation evidence, full local verification, both Tier-1 passes, and the local Pillow/shellcheck caveats. Next: wait for all GitHub CI jobs; if green, request user approval to mark ready and squash-merge.
