...
**Current milestone:** 🚧 IN PROGRESS — **I-1 Web UI i18n (en + zh-Hans)**, claimed 2026-09-09 on `claude/i18n-zh-hans`. This is the 2026-05-15 design spec's §10 stage 8, which never landed: implementation renumbered the stages (the shipped "Stage 8" was the Thumbnail-detect UI), so a stated user requirement fell off every board for ~4 months. Baseline before this work: F-H7 merged as PR #25 (`7d702fc`), full `make test` + `make build` + gofmt/vet re-verified green on 2026-09-09, last CI on `main` (`30568064713`) green on all four jobs.
...
| # | 任务 | owner | status | 备注 |
|---|------|-------|--------|------|
| I-1 | Web UI i18n (en + zh-Hans) | claude@macmini-yiqi | in-progress | Claimed 2026-09-09; `agent-team handoff-check i18n-zh-hans` found no existing claim. Form: bilingual with a switcher (user decision 2026-09-09), resolution `lang` cookie → `--lang` flag → `Accept-Language` → `en` (the spec's `settings.json` layer is dropped — stage 9 Settings was never implemented). Architecture: one pre-parsed template set per locale with `t` bound at parse time; the 24 `ExecuteTemplate` sites become `s.tmplFor(r)`, no view struct changes. Catalog key-set mismatch panics at startup (same posture as the existing template-parse panic) and a scanner test pins template/Go key coverage. In scope: template chrome, user-triggerable 4xx, live-progress inline JS incl. the bash `phase` token. Out of scope: debug pages, raw log stream. Design spec: `docs/superpowers/specs/2026-09-09-twincut-i18n-design.md`. |
