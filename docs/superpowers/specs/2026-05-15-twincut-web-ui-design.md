...
// emitted as quarantine/delete actions complete (or would-be-completed)
// `dry_run` field distinguishes intent (true) from committed action (false).
// `reason` is set for skip kinds: "excluded" | "hardlink"
{"type": "action", "kind": "move" | "delete" | "skip",
 "src": "...", "dst": "...", "matched": "...", "decision": "...",
...
- Orthogonal to `--dry-run`: `dup_group` events fire in both modes. `action` events also fire whenever the script invokes `qmove`/`qdelete`, with the `dry_run` flag distinguishing intent from commit. (The self-check intra-source path skips `qmove` entirely in dry-run, so it emits `dup_group` only — that asymmetry follows the script's existing report-vs-fix split and is fine for the UI: dup_group is always the source of truth for "what would happen.")
