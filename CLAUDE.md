...
- **Video matching has three tiers**, controlled by `VIDEO_FAST` / `VIDEO_FAST_STRICT` / `EXACT`:
  ...
  3. `--video-fast-strict`: tightens windows (size 0.2%, dur 0.15s), additionally compares fps/bitrate in the metadata-index join, and runs one labeled `vid_eq.sh --fast` metadata check per candidate evaluation under those tightened thresholds. Strict defaults are reapplied after CLI parsing.
